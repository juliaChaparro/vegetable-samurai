...
const edit = async (req: Request, res: Response) => {
    try {
        const major = await MajorService.findById((req.params.id as string) ?? "");
        if (!major) {
...
const update = async (req: Request, res: Response) => {
    try {
        ...
        await MajorService.update((req.params.id as string) ?? "", { name, code, description });
        res.redirect("/major");
    } catch (error) {
        console.error(error);
...
const destroy = async (req: Request, res: Response) => {
    try {
        await MajorService.delete((req.params.id as string) ?? "");
